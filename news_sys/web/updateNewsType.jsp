...
<div id="message">
    ...
    <fieldset class="table-search-fieldset">
        <form class="layui-form layui-form-pane" action="/NewsTypeBean?action=query" method="post">
            <div class="layui-inline">
                <label class="layui-form-label">新闻类型名</label>
    ...
    <div id="myModal" class="modal">
        ...
        <div class="modal-content">
            ...
            <form class="layui-form" action="/NewsTypeBean?action=add" method="post">
                <div class="layui-form-item">
                    <label class="layui-form-label">新闻类型</label>
                    <div class="layui-input-block">
                        <input style="width: 200px" type="text" name="typeName" required lay-verify="required"
                               placeholder="请输入新闻类型名"
    ...
    <form action="/NewsTypeBean?action=update" method="post">
        <table class="layui-table">
            ...
            <tbody>
            <%
                Vector<newsType> allNewsType = (Vector<newsType>) session.getAttribute("allNewsType");
                Iterator<newsType> iterator = allNewsType.iterator();
                newsType newsType = new newsType();
                while (iterator.hasNext()) {
                    newsType = iterator.next();
                    if (newsType.getTypeId() == Integer.parseInt(request.getParameter("typeId"))) {
            %>
            <tr>
                <td><input style="width: 80px" type="hidden" name="typeId" value="<%= newsType.getTypeId()%>"><%= newsType.getTypeId()%>
                </td>
                <td><input style="width: 80px" type="text" name="typeName" value="<%= newsType.getTypeName()%>">
                </td>
                <td><input style="width: 130px" type="date" name="createDate" value="<%= newsType.getCreateDate()%>">
                </td>
                <td>
...
<script>
    ...
    function deleteAll() {
        ...
        for (var i = 0; i < oCheck.length; i++) {
            if (oCheck[i].checked) {
                flag = true;
                allTypeId.push(oCheck[i].value);
            }
        }
        if (flag == true) {
            if (confirm("温馨提示：您是否确定删除这些记录，记录删除后将不可还原！")) {
                location.href = "/NewsTypeBean?action=delete&flag=all&TypeId=" + allTypeId;
            }
        } else {
