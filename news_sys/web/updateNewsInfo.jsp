...
<div id="message">
    ...
    <form action="/NewsInfoBean?action=update" method="post">
        ...
        <table class="layui-table">
            ...
            <tbody>
            <%
                Vector<newInfo> allNewInfo = (Vector<newInfo>) session.getAttribute("allNewInfo");
                Iterator<newInfo> iterator = allNewInfo.iterator();
                newInfo newinfo=new newInfo();
                while (iterator.hasNext()) {
                    newinfo = iterator.next();
                    if (newinfo.getNewsId().equals(request.getParameter("NewsId"))) {
            %>
            <tr>
                <td><input style="width: 10px" type="hidden" name="newsId" value="<%= newinfo.getNewsId()%>"><%= newinfo.getNewsId()%>
                </td>
                <td><textarea style="width: 80px" type="text" name="newsTitle" value="<%= newinfo.getNewsTitle()%>"><%= newinfo.getNewsTitle()%></textarea>
                </td>
                <td><textarea style="width: 60px" type="text" name="newsContent" value="<%= newinfo.getNewsContent()%>"><%= newinfo.getNewsContent()%></textarea>
                </td>
                <td><input style="width: 80px" type="text" name="typeId" value="<%= newinfo.getTypeId() %>">
                </td>
                <td><input style="width: 130px" type="date" name="newsPublish" value="<%= newinfo.getNewsPublish()%>">
                </td>
                <td><textarea style="width: 160px" type="text" name="newsSource" value="<%= newinfo.getNewsSource()%>"><%= newinfo.getNewsSource()%></textarea>
                </td>
                <td><textarea style="width: 100px" type="text" name="remark" value="<%= newinfo.getRemark()%>"><%= newinfo.getRemark()%></textarea>
                </td>
                <td>
